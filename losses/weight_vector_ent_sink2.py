...
class Weight_Vec_Ent_Sink2(nn.Module):
    def __init__(self, dis_mtx_density, dis_mtx_density2, dis_mtx_density12, sigma, device):
        super(Weight_Vec_Ent_Sink2, self).__init__()
        self.dis_mtx_density = dis_mtx_density/sigma
        self.dis_mtx_density2 = dis_mtx_density2/sigma
        self.dis_mtx_density12 = dis_mtx_density12/sigma
        self.sigma = sigma
        self.device = device


    def forward(self, pre_density, target_mass, show_res=False):
        g_cross = torch.zeros(len(self.dis_mtx_density2), dtype=torch.float32, device=self.device)
        ...
        q_auto = torch.zeros(len(self.dis_mtx_density2), dtype=torch.float32, device=self.device)
        for _ in range(20):
            p_auto_pre = p_auto
        ...
        for _ in range(20):
            q_auto_pre = q_auto
            autog_pow_dis = self.dis_mtx_density2 - q_auto[:,None]
            q_auto = 0.5 * (q_auto + c_transform(autog_pow_dis, target_mass))
            if self.sigma * torch.mean(torch.abs(q_auto-q_auto_pre))< 1e-4:
                break

        for _ in range(200):
            g_cross_pre = g_cross
            f_cross_pre = f_cross
            crossg_pow_dis = self.dis_mtx_density12 - g_cross[:,None]
            f_cross = c_transform(crossg_pow_dis, target_mass)
            f_cross = f_cross / (1 + self.sigma)
            crossf_pow_dis = self.dis_mtx_density12.transpose(1,0) - f_cross[:,None]
            g_cross = c_transform(crossf_pow_dis, pre_density)
            g_cross = g_cross / (1 + self.sigma)
